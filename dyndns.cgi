...
#   1) List rules (-L) from table $TABLE with line numbers; do not use reverse
#      DNS (-n)
#   2) Find all entries that allow access to DNS port (udp 53).
...
RULENUMBER=$( \
  iptables --line-numbers -nL $TABLE 2>/dev/null \
  | grep -P "^[\d\s]+ACCEPT\s+udp  --  [\d\.\/\s]+ udp dpt:53$" \
  | grep -Pv "172[\.\d]+/16" \
  | grep -Pvm 1 "127.0.0.1" \
  | grep -Po "^\d+")
iptables -D $TABLE $RULENUMBER
iptables -I $TABLE $RULENUMBER --proto udp --dport 53 -s $NEWIP -j ACCEPT
